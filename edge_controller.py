    ...
    while _stop_event.is_set() == False:
        ...
        for p_id in order:
            ...
            for i, (pred, slicing_index) in enumerate(p.input_slicing.items()):
                ...
                schedule = torch.tensor([dataset.partition_layer_map[p_id], len(p.input_slicing), src, dst, p.input_height, p.input_width, p.input_channel, slicing_index[0], slicing_index[1], tag, proc_flag], dtype=torch.int32)
                tag += 1
                # dst는 데이터를 받는 역할을 함
                # 데이터의 dst에 스케줄 보냄
                if dst == 0:
                    ...
                    with recv_schedule_lock:
                        recv_schedule_list.append(copy.deepcopy(schedule))
                else:
                    schedule[2] = src
                    schedule[3] = -1
                    send_schedule(schedule=copy.deepcopy(schedule), dst=dst)
                # print(schedule)
                # src는 데이터를 보내는 역할을 함
                # 데이터의 src에 스케줄 보냄
                if src == 0:
                    ...
                    with send_schedule_lock:
                        send_schedule_list.append(copy.deepcopy(schedule))
                else:
                    schedule[2] = -1
                    schedule[3] = dst
                    send_schedule(schedule=copy.deepcopy(schedule), dst=src)
                del schedule
