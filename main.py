...
from genetic import Genetic, PSOGA, Greedy
...
def result(dataset, x_lsts, took, algorithm_name):
    print("\033[95m---------- " + algorithm_name + " Test! ----------\033[96m")
    result = []
    for idx, x_lst in enumerate(x_lsts):
        total_time = []
        total_energy = []
        total_reward = []
...
if __name__=="__main__":
    ...
    try:
        ...
        test_dataset = DAGDataSet(num_timeslots=1, num_services=3, net_manager=net_manager, apply_partition=False)

    except:
        test_dataset = DAGDataSet(num_timeslots=1, num_services=3, apply_partition=False)
    ...
    for num_services in range(service_low, service_high+1, service_step):
        ...
        genetic = Genetic(dataset=dataset, num_solutions=50, mutation_ratio=0.1, cross_over_ratio=0.9)
        ...
        for num_servers in range(server_low, server_high+1, server_step):
            ...
            # memetic_x_lst = [x for (x, e, t) in temp]
            # memetic_eval_lst.append([e for (x, e, t) in temp])
            # memetic_took = [t for (x, e, t) in temp]
            # memetic_result.append(sorted(result(dataset, memetic_x_lst, took=memetic_took, algorithm_name="Memetic Algorithm (M={}, D={})".format(num_services, num_servers)), key=itemgetter(2), reverse=True))
        ...
        result_by_services.append([local_result, edge_result, greedy_result, memetic_result, psoga_result, genetic_result, memetic_eval_lst, psoga_eval_lst, genetic_eval_lst, server_low, server_high, server_step, num_services])
