...
class Worker(mp.Process):
    def __init__(self, id, env, gamma, buffer_size, batch_size, global_policy_network, global_value_network, global_policy_optimizer, global_value_optimizer, global_episode, GLOBAL_MAX_EPISODE):
    ...
    def get_action(self, c_id, state):
    ...
    def compute_loss(self, trajectory):
        ...
        discounted_rewards = [torch.sum(torch.FloatTensor([pow(self.gamma, idx) * reward for idx, reward in enumerate(rewards[j:])]).to(self.device)) for j in range(rewards.size(0))]
    ...
    def run(self):
        ...
        while self.global_episode.value < self.GLOBAL_MAX_EPISODE:
            ...
            one_hot_action, action, mask = self.get_action(c_id, state)
