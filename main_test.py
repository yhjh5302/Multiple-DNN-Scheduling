...
from genetic import Genetic, PSOGA, Greedy, HEFT, CPOP, PEFT
...
if __name__=="__main__":
    ...
    for num_services in range(service_low, service_high+1, service_step):
        ...
        dataset = DAGDataSet(num_timeslots=1, num_services=num_services, net_manager=test_dataset.system_manager.net_manager, apply_partition=False)
        ...
        psoga = PSOGA(dataset=dataset, num_particles=100, w_max=0.8, w_min=0.2, c1_s=0.9, c1_e=0.2, c2_s=0.4, c2_e=0.9)
        genetic = Genetic(dataset=dataset, num_solutions=100, mutation_ratio=0.1, cross_over_ratio=0.9)
        ...
        for num_servers in range(server_low, server_high+1, server_step):
            ...
            # greedy_psoga_x_lst = [x for (x, e, t) in temp]
            # greedy_psoga_eval_lst.append([e for (x, e, t) in temp])
            # greedy_psoga_took = [t for (x, e, t) in temp]
            # greedy_psoga_took_lst.append(greedy_psoga_took)
            # greedy_psoga_result.append(sorted(result(dataset, greedy_psoga_x_lst, took=greedy_psoga_took, algorithm_name="Greedy-PSO-GA Algorithm (M={}, D={})".format(num_services, num_servers)), key=itemgetter(2), reverse=True))

            # temp = [genetic.run_algo(loop=loop, verbose=False, local_search=True) for _ in range(iteration)]
            # greedy_genetic_x_lst = [x for (x, e, t) in temp]
            # greedy_genetic_eval_lst.append([e for (x, e, t) in temp])
            # greedy_genetic_took = [t for (x, e, t) in temp]
            # greedy_genetic_took_lst.append(greedy_genetic_took)
            # greedy_genetic_result.append(sorted(result(dataset, greedy_genetic_x_lst, took=greedy_genetic_took, algorithm_name="Greedy-Genetic Algorithm (M={}, D={})".format(num_services, num_servers)), key=itemgetter(2), reverse=True))
        ...
        with open("outputs/test_results_backup_{}".format(num_services), "wb") as fp:
            pickle.dump(result_by_services, fp)

    with open("outputs/test_results_backup", "wb") as fp:
        pickle.dump(result_by_services, fp)

    for result_by_servers in result_by_services:
        draw_result.by_num_service_test(*result_by_servers)