...
def by_num_service(local_result, edge_result, heft_result, greedy_result, memetic_psoga_result, memetic_genetic_result, psoga_result, genetic_result, memetic_psoga_eval_lst, memetic_genetic_eval_lst, psoga_eval_lst, genetic_eval_lst, memetic_psoga_took_lst, memetic_genetic_took_lst, psoga_took_lst, genetic_took_lst, server_low, server_high, server_step, num_services):
    # [0,1,2]: 0 - num_devices, 1 - num_loop, 2 - [time,energy,reward]
    
    algorithms = ['HEFT', 'Greedy', 'PSO-GA', 'Genetic', 'Memetic']
    ...
    for idx, algorithm in enumerate(algorithms):
        ...
        if algorithm == 'User Device':
            avg = np.array(local_result)[:,:1,0] * 1000
        elif algorithm == 'Edge':
            avg = np.array(edge_result)[:,:1,0] * 1000
        elif algorithm == 'HEFT':
            avg = np.array(heft_result)[:,:1,0] * 1000
        elif algorithm == 'Greedy':
            avg = np.array(greedy_result)[:,:1,0] * 1000
        elif algorithm == 'Genetic':
            avg = np.array(genetic_result)[:,:1,0] * 1000
        elif algorithm == 'PSO-GA':
            avg = np.array(psoga_result)[:,:1,0] * 1000
        elif algorithm == 'Memetic':
            avg = np.array(memetic_genetic_result)[:,:1,0] * 1000
        elif algorithm == 'Memetic-PSO-GA':
            avg = np.array(memetic_psoga_result)[:,:1,0] * 1000
        else:
...
# Get results by the number of services
def by_num_service_test(local_result, edge_result, heft_result, greedy_result, server_low, server_high, server_step, num_services):
    # [0,1,2]: 0 - num_devices, 1 - num_loop, 2 - [time,energy,reward]
    
    algorithms = ['Edge', 'HEFT', 'Greedy']
